...
def backend_load_nii_slice(nii_path):
    ...
    try:
        ...
        try:
            ...
            mri_id = os.path.basename(nii_path).split('.')[0].replace('_axl', '').strip()
...
def _copiar_arquivos(dataset_df, destino_folder, img_dir_origem):
    ...
    print(f"Criando pasta: {destino_path}")
    ...
    for mri_id in dataset_df['MRI ID']:
        ...
        arquivo_destino = destino_path / nome_arquivo
        
        if arquivo_origem.exists():
            shutil.copy(arquivo_origem, arquivo_destino)
            copiados += 1
        else:
            print(f"  Aviso: Arquivo não encontrado na origem: {arquivo_origem}")
            nao_encontrados += 1
            
    print(f"Cópia para {destino_folder} concluída.")
    print(f"  {copiados} arquivos copiados.")
    print(f"  {nao_encontrados} arquivos não encontrados.")
    print(f"  CSV demográfico salvo em: {csv_path}")
...
def backend_run_db_split():
    ...
    try:
        ...
        if not OASIS_CSV_PATH.exists():
            print(f"Erro: Arquivo demográfico principal não encontrado em {OASIS_CSV_PATH}")
            raise Exception(f"Arquivo não encontrado: {OASIS_CSV_PATH}")
        ...
        print(f"{df.shape[0]} registros válidos (Demented/NonDemented) encontrados.")

        # 3. Divisão Train/Val (80%) e Test (20%)
        ...
        print(f"Divisão secundária: {len(df_train)} para Treino, {len(df_val)} para Validação.")

        # 5. Verificar o diretório de origem das imagens
        if not IMG_DIR_AXL.exists():
            print(f"Erro: Diretório de imagens NIfTI não encontrado em {IMG_DIR_AXL}")
            print("Por favor, verifique se a pasta 'oasis_mri_axl' existe em 'database/'.")
            raise Exception(f"Diretório de origem das imagens não encontrado: {IMG_DIR_AXL}")
        print(f"Diretório de origem das imagens (NIfTI) verificado: {IMG_DIR_AXL}")

        # 6. Copiar arquivos para os respectivos diretórios
        _copiar_arquivos(df_train, "treino", IMG_DIR_AXL)
        _copiar_arquivos(df_val, "validacao", IMG_DIR_AXL)
        _copiar_arquivos(df_test, "teste", IMG_DIR_AXL)
        
        print("\nDivisão do banco de dados concluída com sucesso!")
        return True
        
    except Exception as e:
        print(f"Erro crítico durante a divisão do banco de dados: {e}")
        QMessageBox.critical(None, "Erro na Divisão da Base", f"Ocorreu uma falha: {e}")
        return False
...
def _processar_dataset_features(set_name):
    ...
    set_dir = DATABASE_DIR / set_name
    csv_path = set_dir / "features_full.csv"
    ...
    if 'MRI ID' in df_demographics.columns:
        ...
        print("Aviso: Coluna 'MRI ID' não encontrada, 'MRI_ID' já deve existir.")
    ...
    for i, row in enumerate(df_demographics.itertuples()):
        ...
        # Ignora arquivos que não foram encontrados pelo db_split
        img_path = set_dir / f"{mri_id}_axl.nii.gz"
        if not img_path.exists():
            print(f"  Aviso: Pulando {mri_id} (arquivo NIfTI não encontrado em {set_dir}).")
            continue
            
        if (i + 1) % 10 == 0 or i == 0: # Imprime o progresso
        ...
        try:
            ...
            # Converter a tupla nomeada (row) para um dicionário
            metadata_dict = row._asdict()
            metadata_dict.update(features_dict)
            all_features_list.append(metadata_dict)
            
        except Exception as e:
            print(f"  ERRO ao processar {mri_id}: {e}. Pulando este arquivo.")
            
    # 4. Criar o DataFrame final com features
    df_final_features = pd.DataFrame(all_features_list)
    
    # Remove a coluna 'Index' que o itertuples adiciona
    ...
    print(f"Processamento de {set_name} concluído. {len(df_final_features)} arquivos com features extraídas.")
    
    # 6. Sobrescreve o arquivo 'features_full.csv' com os dados completos
    df_final_features.to_csv(csv_path, sep=';', decimal=',', index=False)
    print(f"Arquivo 'features_full.csv' atualizado com features em: {set_dir}")
...
def backend_run_feature_extraction():
    ...
    try:
        ...
        # Processa os três conjuntos em sequência
        _processar_dataset_features("treino")
        _processar_dataset_features("validacao")
        _processar_dataset_features("teste")
        
        print("\nExtração de features concluída com sucesso!")
        return True
        
    except Exception as e:
        print(f"Erro crítico durante a extração de features: {e}")
        QMessageBox.critical(None, "Erro na Extração de Features", f"Ocorreu uma falha: {e}")
        return False
...
class ImageProcessingApp(QMainWindow):
    def __init__(self):
        ...
        print("Use os botões 'Dividir Base' -> 'Extrair Features' -> 'Treinar Modelos' para configurar a aplicação pela primeira vez.")
    ...
    def criar_barra_superior(self):
        ...
        layout_botoes_principais.addWidget(btn_selecionar_img)
        ...
        layout_botoes_principais.addWidget(btn_menu_graficos)
        
        layout_superior.addLayout(layout_botoes_principais)
        
        # --- Botões de Gerenciamento da Base ---
        # Separador
        v_line_db = QFrame()
        v_line_db.setFrameShape(QFrame.Shape.VLine)
        v_line_db.setFrameShadow(QFrame.Shadow.Sunken)
        layout_superior.addWidget(v_line_db)

        # Layout para botões de gerenciamento da base
        layout_botoes_db = QHBoxLayout()
        
        btn_dividir_db = QPushButton("Dividir Base de Dados")
        btn_dividir_db.clicked.connect(self.confirmar_dividir_db)
        layout_botoes_db.addWidget(btn_dividir_db)
        
        btn_extrair_features = QPushButton("Extrair Features (Base)")
        btn_extrair_features.clicked.connect(self.confirmar_extracao_features)
        layout_botoes_db.addWidget(btn_extrair_features)

        btn_treinar_modelos = QPushButton("Treinar Modelos")
        btn_treinar_modelos.clicked.connect(self.confirmar_treinamento)
        layout_botoes_db.addWidget(btn_treinar_modelos)
    ...
    def confirmar_dividir_db(self):
        titulo = "Confirmar Divisão da Base de Dados"
        texto = ("Este processo irá apagar e recriar as pastas 'treino', 'validacao' e 'teste' "
                 "em 'database/', copiando os arquivos NIfTI de 'oasis_mri_axl'.\n\n"
                 "Deseja continuar?")
        ...
        if resposta == QMessageBox.StandardButton.Yes:
            self.text_edit_terminal.clear()
            print("Iniciando divisão da base de dados...")
            self.setDisabled(True) 
            QApplication.processEvents() 
            
            try:
                success = backend_run_db_split()
                if success:
                    QMessageBox.information(self, "Concluído", "Base de dados dividida com sucesso.")
                else:
                    QMessageBox.warning(self, "Falha", "A divisão da base de dados falhou. Verifique o console para mais detalhes.")
            except Exception as e:
                 QMessageBox.critical(self, "Erro Crítico na Divisão", f"Falha: {e}")
            
            self.setDisabled(False)

    def confirmar_extracao_features(self):
        titulo = "Confirmar Extração de Features"
        texto = ("Este processo é MUITO DEMORADO.\n"
                 "Ele irá ler cada imagem NIfTI dos conjuntos de treino, validação e teste, "
                 "processar a segmentação, extrair features e sobrescrever os arquivos 'features_full.csv'.\n\n"
                 "Deseja continuar?")
        resposta = QMessageBox.question(self, titulo, texto, 
                                        QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                                        QMessageBox.StandardButton.No)
        ...
        if resposta == QMessageBox.StandardButton.Yes:
            self.text_edit_terminal.clear()
            print("Iniciando extração de features (processo demorado)...")
            self.setDisabled(True) 
            QApplication.processEvents() 
            
            try:
                success = backend_run_feature_extraction()
                if success:
                    QMessageBox.information(self, "Concluído", "Extração de features concluída com sucesso.")
                else:
                    QMessageBox.warning(self, "Falha", "A extração de features falhou. Verifique o console para mais detalhes.")
            except Exception as e:
                 QMessageBox.critical(self, "Erro Crítico na Extração", f"Falha: {e}")
            
            self.setDisabled(False)

    def confirmar_treinamento(self):
        titulo = "Confirmar Treinamento"
        texto = ("Este processo pode demorar alguns minutos e irá sobrescrever os modelos existentes "
                 "na pasta 'models/'.\n"
                 "Certifique-se que a 'Extração de Features' já foi executada.\n\n"
                 "Deseja treinar os modelos rasos novamente?")
        resposta = QMessageBox.question(self, titulo, texto, 
                                        QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                                        QMessageBox.StandardButton.No)
